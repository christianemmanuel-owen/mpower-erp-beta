...
describe('the collection gate', () => {
  ...
  it('pays once the money is recorded as collected', () => {
    const s = sale({ installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' })] })
  ...
  it('pays nothing on a cancelled sale, even if an installment sits collected', () => {
    const s = sale({
      status: 'cancelled',
      installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' })],
    })
...
describe('which period the commission lands in', () => {
  ...
  it('includes a collection on the first and last day of the period', () => {
    const first = sale({ id: 'a', installments: [inst({ status: 'cleared', collectedAt: '2026-08-01T00:00:00.000Z', clearedAt: '2026-08-01T00:00:00.000Z' })] })
    const last = sale({ id: 'b', installments: [inst({ status: 'cleared', collectedAt: '2026-08-31T23:00:00.000Z', clearedAt: '2026-08-31T23:00:00.000Z' })] })
    expect(total([first])).toBe(6_200)
    expect(total([last])).toBe(6_200)
  })

  it('excludes a collection from the following period', () => {
    const s = sale({ installments: [inst({ status: 'cleared', collectedAt: '2026-09-01T00:00:00.000Z', clearedAt: '2026-09-01T00:00:00.000Z' })] })
    expect(total([s])).toBe(0)
  })
  ...
  it('falls back to the due date when no collection timestamp was recorded', () => {
    ...
    const s = sale({ installments: [inst({ status: 'cleared', dueDate: '2026-08-10T00:00:00.000Z' })] })
    expect(total([s])).toBe(6_200)
  })
})
...
describe('proportional release across an installment plan', () => {
  ...
  it('releases only the share that has been collected', () => {
    const s = plan([{ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' }])
    expect(total([s])).toBe(1_860) // 93,000 × 2%
  })

  it('releases the rest when the final installment lands', () => {
    const s = plan([
      { status: 'cleared', collectedAt: '2026-07-12T00:00:00.000Z', clearedAt: '2026-07-12T00:00:00.000Z' },
      { status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' },
    ])
  ...
  it('sums to the full commission once every installment is collected', () => {
    const s = plan([
      { status: 'cleared', collectedAt: '2026-08-05T00:00:00.000Z', clearedAt: '2026-08-05T00:00:00.000Z' },
      { status: 'cleared', collectedAt: '2026-08-20T00:00:00.000Z', clearedAt: '2026-08-20T00:00:00.000Z' },
    ])
    expect(total([s])).toBe(6_200)
  })
})
...
describe('what is commissionable', () => {
  it('does not pay commission on financing interest', () => {
    ...
    const s = sale({
      installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z', principal: 310_000, interestPct: 10, amount: 341_000 })],
    })
    expect(total([s])).toBe(6_200)
  })
  ...
  it('scales per-litre commission to a part-collected plan', () => {
    const s = sale({
      installments: [
        inst({ id: 'a', principal: 155_000, amount: 155_000, status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' }),
  ...
  it('only counts the agent asked for', () => {
    const mine = sale({ id: 'a', installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' })] })
    const theirs = sale({ id: 'b', agentId: 'ag2', installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z' })] })
  ...
  it('survives a zero-value sale without dividing by zero', () => {
    const s = sale({
      ...
      installments: [inst({ status: 'cleared', collectedAt: '2026-08-12T00:00:00.000Z', clearedAt: '2026-08-12T00:00:00.000Z', principal: 0, amount: 0 })],
    })
    expect(total([s])).toBe(0)
...
describe('commissionLines', () => {
  it('shows its working, oldest collection first', () => {
    const s = sale({
      installments: [
        inst({ id: 'b', principal: 155_000, amount: 155_000, status: 'cleared', collectedAt: '2026-08-20T00:00:00.000Z', clearedAt: '2026-08-20T00:00:00.000Z' }),
        inst({ id: 'a', principal: 155_000, amount: 155_000, status: 'cleared', collectedAt: '2026-08-05T00:00:00.000Z', clearedAt: '2026-08-05T00:00:00.000Z' }),
      ],
    })
